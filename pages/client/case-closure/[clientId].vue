...
/**
 * A client's ទម្រង់ទី៦ closures. Many per client: a closure is terminal for that
 * episode, but a client can be re-admitted and closed again.
 */
...
/**
 * ទម្រង់ទី៦ may only be started once the form before it has been sent for
...
const mayStart = computed(
  () => !client.value?.pipeline || mayStartForm(client.value.pipeline, 6)
);
...
const outcomeLabel = (v?: string | null) =>
  CLOSURE_OUTCOME.find((o) => o.value === v)?.label ?? "—";

/** The reason, whichever group it came from. */
const reasonOf = (r: any) => {
  ...
  const list = String(r?.failureReasons ?? "").split(",").filter(Boolean);
  return list.length ? `${list.length} មូលហេតុ` : "—";
};
...
onMounted(async () => {
  try {
    const [c, list]: any = await Promise.all([
      ...
      $fetch("/api/client/case-closure/get", { method: "POST", body: { clientId: clientId.value } }),
    ]);
...
/**
 * Removes one ទម្រង់ទី៦ record. The shared endpoint takes its approval history
 * with it. Closing never changed the client record, so reopening a case is just
 * removing the closure — which is what the confirmation says.
 *
...
const removeRecord = async (r: any, i: number) => {
  ...
  if (!(await confirmDelete(
    `លុបកំណត់ត្រាការបិទករណី ថ្ងៃទី ${fmt(r.createdAt)} ប៉ុន្តែកំណត់ត្រាអតិថិជននឹងមិនផ្លាស់ប្តូរទេ។`
  ))) return;

  try {
    await $fetch("/api/client/case-closure/delete", { method: "POST", body: { id: r.id } });
  ...
  <div class="font-[Battambang]">
    <div class="mt-5">
      <div class="flex items-start justify-between gap-4">
        ...
        <NuxtLink v-if="client && mayStart" :to="`/client/case-closure/form?client=${client.id}`">
          ...
          <table class="w-full text-left text-base">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
              <tr v-for="(r, i) in rows" :key="r.id">
                ...
                <td class="py-3 pr-4 text-gray-800 dark:text-gray-100">{{ fmt(r.createdAt) }}</td>
                <td class="py-3 pr-4 text-gray-800 dark:text-gray-100">{{ outcomeLabel(r.outcome) }}</td>
                <td class="py-3 pr-4 text-gray-800 dark:text-gray-100">{{ reasonOf(r) }}</td>
                <td class="py-3 pr-4">

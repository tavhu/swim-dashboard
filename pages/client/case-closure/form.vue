  ...
  <div class="font-[Battambang]">
    <div class="mt-5">
      <div class="flex items-start justify-between gap-4">
        <h2 class="text-2xl font-[Moul] text-primary">{{ closureId ? tr('កែសម្រួលការបិទករណី') : tr('បិទករណី') }}</h2>
        ...
        <section class="col-span-12 rounded-lg bg-white p-4 shadow dark:bg-gray-800">
          ...
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label v-for="o in CLOSURE_OUTCOME" :key="o.value"
              ...
              <span>
                <span class="block text-base text-gray-800 dark:text-gray-100">{{ tr(o.label) }}</span>
          ...
          <div v-if="successful" class="mt-4">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ tr('មូលហេតុ (ជ្រើសរើសមួយ)') }}</span>
            ...
            <label v-if="needsOtherText" class="mt-3 block">
              <span class="text-sm text-gray-500 dark:text-gray-400">{{ tr('បញ្ជាក់') }}</span>
          ...
          <div v-else class="mt-4">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ tr('មូលហេតុ (ជ្រើសរើសបានច្រើន)') }}</span>
            <div class="mt-2 space-y-2">
        ...
        <section class="col-span-12 rounded-lg bg-white p-4 shadow dark:bg-gray-800">
          ...
          <div class="mt-2 grid grid-cols-1 gap-4 2xl:grid-cols-3">
            <label v-for="f in CLOSURE_SUMMARY_FIELDS" :key="`centre${f.key}`" class="block">
              <span class="text-sm text-gray-500 dark:text-gray-400">{{ tr(f.label) }}</span>
          ...
          <label class="mt-6 block">
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ tr('គ. គម្រោងអនាគតរបស់ជនរងគ្រោះគួទទួលបាន') }}</span>

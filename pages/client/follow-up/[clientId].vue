...
useHead(() => ({ title: tr("ការតាមដាន និងវាយតម្លៃ") }));
...
/**
 * Removes one ទម្រង់ទី៥ record. The row is the whole episode — the shared endpoint
...
const removeRecord = async (r: any, i: number) => {
  ...
  if (!(await confirmDelete(
    `លុបកំណត់ត្រាការតាមដាន ថ្ងៃទី ${fmt(r.monitorDate)} រួមទាំងបញ្ជីសេវា និងឯកសារភ្ជាប់។`
  ))) return;

  try {
    await $fetch("/api/client/follow-up/delete", { method: "POST", body: { id: r.id } });
  ...
  <div class="font-[Battambang]">
    <div class="mt-5">
      <div class="flex items-start justify-between gap-4">
        <div>
          <h2 class="text-2xl font-[Moul] text-primary">{{ tr('ការតាមដាន និងវាយតម្លៃ') }}</h2>
      ...
      <div v-else class="rounded-lg bg-white p-4 shadow dark:bg-gray-800">
        <p v-if="!rows.length" class="py-8 text-center text-base text-gray-500 dark:text-gray-400">{{ tr('មិនទាន់មានការតាមដាន និងវាយតម្លៃនៅឡើយទេ។') }}</p>
          ...
          <table class="w-full text-left text-base">
            <thead class="border-b text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th class="py-3 pr-4 font-normal">{{ tr('ប្រភេទ') }}</th>
                <th class="py-3 pr-4 font-normal">{{ tr('វិធីសាស្រ្ត') }}</th>
                <th class="py-3 pr-4 font-normal">{{ tr('តាមដានបន្ត') }}</th>

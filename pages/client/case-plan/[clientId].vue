...
/**
 * ទម្រង់ទី៣ may only be started once the form before it has been sent for
...
const mayStart = computed(
  () => !client.value?.pipeline || mayStartForm(client.value.pipeline, 3)
);
...
useHead(() => ({ title: tr("ផែនការករណីរបស់អតិថិជន") }));
...
/**
 * Removes one ទម្រង់ទី៣ record. The row is the whole plan — its activities
 * cascade and the shared endpoint takes its approval history with it, so the
 * confirmation says as much rather than asking a bare "are you sure?".
 *
...
const removeRecord = async (r: any, i: number) => {
  ...
  if (!(await confirmDelete(
    `លុបកំណត់ត្រាផែនការករណី ថ្ងៃទី ${fmt(r.createdAt)} រួមទាំងសកម្មភាពទាំងអស់។`
  ))) return;

  try {
    await $fetch("/api/client/case-plan/delete", { method: "POST", body: { id: r.id } });
  ...
  <div class="font-[Battambang]">
    <div class="mt-5">
      <div class="flex items-start justify-between gap-4">
        ...
        <NuxtLink v-if="client && mayStart" :to="`/client/case-plan/form?client=${client.id}`">
      ...
      <div v-else class="rounded-lg bg-white p-4 shadow dark:bg-gray-800">
        <p v-if="!rows.length" class="py-8 text-center text-base text-gray-500 dark:text-gray-400">{{ tr('មិនទាន់មានផែនការករណីនៅឡើយទេ។') }}</p>
          ...
          <table class="w-full text-left text-base">
            <thead class="border-b text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th class="py-3 pr-4 font-normal">{{ tr('ស្ថាប័នទទួលផ្តល់សេវា') }}</th>
                ...
                <th class="py-3 pr-4 font-normal">{{ tr('ចំនួនសកម្មភាព') }}</th>
                <th class="py-3 pr-4 font-normal">{{ tr('កាលបរិច្ឆេទតាមដាន') }}</th>
